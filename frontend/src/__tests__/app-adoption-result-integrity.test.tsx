...
describe('App adoption result integrity', () => {
  ...
  it('marks unavailable management categories honestly and restores memorial trigger focus', async () => {
    ...
    expect(screen.queryByRole('group', { name: '地图模式' })).toBeNull()
    expect(screen.queryByRole('group', { name: '地图镜头' })).toBeNull()
    const mapToolDock = screen.getByRole('navigation', { name: '地图工具入口' })
    expect(controlRail?.contains(mapToolDock)).toBe(true)
    expect(Array.from(mapToolDock.querySelectorAll('button')).map((button) => button.textContent)).toEqual(['地图', '镜头'])
